    ...
    <section id="case-studies" className="py-20 md:py-32 px-4 sm:px-6 lg:px-8 bg-zinc-950/50 scroll-mt-20">
      <ScrollReveal>
        <div className="max-w-6xl mx-auto">
          ...
          <StaggerList className="grid lg:grid-cols-2 gap-8">
            {caseStudies.map((cs, index) => {
              const isLastOdd = index === caseStudies.length - 1 && caseStudies.length % 2 !== 0
              return (
                <StaggerItem key={cs.slug} className={isLastOdd ? 'lg:col-span-2' : undefined}>
                  <article className="border border-white/5 bg-zinc-900/40 rounded-xl p-7 space-y-6 hover:border-emerald-500/20 transition-colors">
                    <div>
